...
def main(args):
    ...
    for case_id, case_row in progress_bar:
        ...
        cleaned_text = case_row["cleaned_text"]
        try:
            ...
            progress_bar.set_description(f"Case {case_id}: Extracting style and preferences")
            json_style_and_preference = gemini.predict(
                system_instruction_semantic_and_style_extraction,
                cleaned_text,
                args.temperature,
                None,
            ...
            if case_id % 5 == 0:
                logger.info(f"Case {case_id}: Generating Report using style and preferences: {json_style_and_preference}")
            study_prompt = f"STUDY NAME: {case_row['study_description']}\nJSON STYLE AND PREFERENCES: {json_style_and_preference}"

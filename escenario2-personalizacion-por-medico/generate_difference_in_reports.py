...
from src.system_instructions.escenario2_personalizacion_por_medico import system_instruction_compare_two_medics
...
def main(args):
    ...
    dataset_metadata = pd.read_csv(args.metadata_path).loc[0:3, :]
    ...
    for case_id, case_row in progress_bar:
        progress_bar2 = tqdm(dataset_metadata.loc[dataset_metadata.index[case_id::]].iterrows(), total=len(dataset_metadata), leave=False)
        ...
        for case_id2, case_row2 in progress_bar2:
            ...
            try:
                prompt = f"MEDIC A:\n{clean_text1}\nMEDIC B:\n{clean_text2}\n"
                result = gemini.predict(system_instruction_compare_two_medics, prompt, args.temperature, None)

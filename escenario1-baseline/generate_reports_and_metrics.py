...
from src.system_instructions.escenario1_baseline import system_instruction_report_generator, \
...
def main(args):
    ...
    for case_id, case_row in progress_bar:
        ...
        try:
            image_path = os.path.join(args.images_path, case_row["file_url"].replace("dcm", "jpg"))
            if not os.path.exists(image_path):
                 logger.warning(f"Image not found at {image_path}, skipping case {case_id}")
                 continue
            ...
            generated_report = gemini.predict(system_instruction_report_generator, study_description, args.temperature, image)
            ...
            if use_metric1:
                ...
                dataset_metadata.at[case_id, "metric1_json_generated"] = gemini.predict(system_instruction_text_json_normalizer, generated_report, args.temperature, None)
                dataset_metadata.at[case_id, "metric1_json_real"] = gemini.predict(system_instruction_text_json_normalizer, case_row["cleaned_text"], args.temperature, None)
                metric1_prompt = metric1_prompt_merge(dataset_metadata.at[case_id, "metric1_json_real"], dataset_metadata.at[case_id, "metric1_json_generated"])
                dataset_metadata.at[case_id, "metric1_json_result"] = gemini.predict(system_instruction_jsons_comparison, metric1_prompt, args.temperature, None)

            if use_metric2:
                progress_bar.set_description(f"Case {case_id}: Calculating Metric 2")
                metric2_prompt = metric2_prompt_merge(case_row["cleaned_text"], generated_report)
                dataset_metadata.at[case_id, "metric2_json_result"] = gemini.predict(system_instruction_text_to_text_comparison, metric2_prompt, args.temperature, None)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--images-path", type=str, required=True, help="Path to dataset")
    parser.add_argument("--metadata-path", type=str, required=True, help="Path to metadata")
    parser.add_argument("--metrics", type=str, default="all", choices=["all", "metric1", "metric2"], help="Metrics to evaluate")
    parser.add_argument("--output-path", type=str, required=True, help="Path to output")
    parser.add_argument("--temperature", type=float, default=0., help="Temperature for Gemini")
